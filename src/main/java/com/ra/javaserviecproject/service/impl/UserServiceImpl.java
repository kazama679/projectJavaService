...
import com.ra.javaserviecproject.service.authJwt.RoleService;
...
public class UserServiceImpl implements UserService {
    ...
    public UserRegisterResponse register(UserRegisterDTO userRegisterDTO) {
        ...
        roles.add(roleRepository.findByRoleName("STUDENT"));
        User user = User
                .builder()
    ...
    @Override
    public User update(UserUpdateDTO user, Integer id) {
        User findUser = userRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Không tìm thấy người dùng với ID: " + id));
        if (user.getEmail() != null && !user.getEmail().equals(findUser.getEmail())
                && userRepository.existsByEmailAndIdNot(user.getEmail(), id)) {
            throw new RuntimeException("Email đã được sử dụng");
        }
        if (user.getUsername() != null && !user.getUsername().equals(findUser.getUsername())
                && userRepository.existsByUsernameAndIdNot(user.getUsername(), id)) {
            throw new RuntimeException("Username đã được sử dụng");
        }
        if (user.getPhoneNumber() != null && !user.getPhoneNumber().equals(findUser.getPhoneNumber())
                && userRepository.existsByPhoneAndIdNot(user.getPhoneNumber(), id)) {
            throw new RuntimeException("Số điện thoại đã được sử dụng");
        }
